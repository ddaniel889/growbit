...
const battlesCheckSendCreateData = (data) => {
  if (data === undefined || data === null) {
    ...
    isNaN(data.playerCount) === true ||
    Math.floor(data.playerCount) < 2 ||
    Math.floor(data.playerCount) > 4
  ) {
    ...
    ["standard", "team", "group"].includes(data.mode) !== true
  ) {
    throw new Error("Your entered mode is invalid.");
  } else if (
    data.boxes === undefined ||
    data.boxes === null ||
    Array.isArray(data.boxes) !== true ||
    data.boxes.length <= 0
  ) {
    ...
    data.affiliateOnly === undefined ||
    data.affiliateOnly === null ||
    typeof data.affiliateOnly !== "boolean"
  ) {
    throw new Error("Your entered affiliate value is invalid.");
  } else if (
    data.cursed === undefined ||
    data.cursed === null ||
    typeof data.cursed !== "boolean"
  ) {
    throw new Error("Your entered affiliate value is invalid.");
  } else if (
...
const battlesCheckSendCreateBoxes = (data) => {
  ...
  for (let box of data.boxes) {
    if (box._id === undefined || typeof box._id !== "string") {
      throw new Error("Your entered box id is invalid.");
    } else if (
      box.count === undefined ||
      isNaN(box.count) === true ||
      Math.floor(box.count) <= 0
    ) {
...
const battlesCheckSendBotGame = (
  ...
  if (
    battlesGame === undefined ||
    battlesGame === null ||
    battlesGame.state !== "created" ||
    battlesBlockGame.includes(battlesGame._id.toString()) === true ||
    battlesGame.playerCount <=
      Math.floor(
        battlesGame.bets.length +
          battlesBlockJoin.filter(
            (element) => element.toString() === battlesGame._id.toString(),
          ).length,
      )
  ) {
...
const battlesCheckSendJoinData = (data) => {
  if (data === undefined || data === null) {
    ...
    data.slot === undefined ||
    data.slot === null ||
    isNaN(data.slot) === true ||
    Math.floor(data.slot) < 1 ||
    Math.floor(data.slot) > 3
  ) {
    throw new Error("You’ve entered an invalid slot.");
  }
};

const battlesCheckSendJoinGame = (
  data,
  user,
  battlesGame,
  battlesBlockJoin,
  ...
  if (
    ...
    battlesGame.bets.some(
      (element) => element.slot === Math.floor(data.slot),
    ) === true
  ) {
    ...
    throw new Error(
      "You aren't allowed to join more then one time per battles game.",
    );
  } else if (
    battlesGame.options.affiliateOnly === true &&
    (user.affiliates.referrer === undefined ||
      user.affiliates.referrer.toString() !==
        battlesGame.bets[0].user._id.toString())
  ) {
    throw new Error(
      "You need to be an creators affiliate to join this battles game.",
    );
  }
};
...
const battlesCheckSendCancelGame = (
  user,
  battlesGame,
  battlesBlockJoin,
  battlesBlockGame,
...
const battlesGetGameIndex = (battlesGames, gameId) => {
  return battlesGames.findIndex(
    (element) => element._id.toString() === gameId.toString(),
  );
};
...
const battlesGetAmountGame = (boxes) => {
  ...
  for (let box of boxes) {
    amount = amount + Math.floor(box.box.casePrice * box.count);
  }
...
const battlesGetAmountWin = (battlesGame) => {
  ...
  for (const bet of battlesGame.bets) {
    for (const [index, outcome] of bet.outcomes.entries()) {
      amount = Math.floor(
        amount +
          battlesGetOutcomeItem(rounds[index].box.items, outcome).itemPrice,
      );
    }
  }
...
const battlesGetWinnerBets = (battlesGame) => {
  ...
  const bets = battlesGame.bets.map((bet) => ({
    ...bet,
    outcomes: bet.outcomes.map(
      (outcome, index) =>
        // battlesGetOutcomeItem(rounds[index].box.items, outcome).amountFixed,
        battlesGetOutcomeItem(rounds[index].box.items, outcome).itemPrice,
    ),
  }));
  let winners = [];
  ...
  if (battlesGame.mode === "group") {
    ...
    const amountOne =
      battlesGame.options.terminal === false
        ? [...bets[0].outcomes, ...bets[1].outcomes].reduce(
            (total, outcome) => total + outcome,
            0,
          )
        : Math.floor(
            bets[0].outcomes[bets[0].outcomes.length - 1] +
              bets[1].outcomes[bets[1].outcomes.length - 1],
          );
    const amountTwo =
      battlesGame.options.terminal === false
        ? [...bets[2].outcomes, ...bets[3].outcomes].reduce(
            (total, outcome) => total + outcome,
            0,
          )
        : Math.floor(
            bets[2].outcomes[bets[2].outcomes.length - 1] +
              bets[3].outcomes[bets[3].outcomes.length - 1],
          );

    if (amountOne === amountTwo) {
      winners = battlesGame.bets;
    } else if (
      (battlesGame.options.cursed === false && amountOne > amountTwo) ||
      (battlesGame.options.cursed === true && amountOne < amountTwo)
    ) {
    ...
    const amounts = bets.map((bet) =>
      battlesGame.options.terminal === false
        ? bet.outcomes.reduce((total, outcome) => total + outcome, 0)
        : bet.outcomes[bet.outcomes.length - 1],
    );
    const amountWin =
      battlesGame.options.cursed === false
        ? Math.max(...amounts)
        : Math.min(...amounts);
    ...
    for (const [index, bet] of bets.entries()) {
      ...
      if (amountWin === amountBet) {
        winners.push(battlesGame.bets[index]);
      }
    }
  }
...
const battlesSanitizeGame = (game, all) => {
  ...
  if (sanitized.state !== "completed") {
    sanitized.fair = { hash: sanitized.fair.hash };
  }

  // Sanitize game boxes if all not true
  if (all !== true) {
    for (let box of sanitized.boxes) {
      box.box.items = [];
    }
  }
