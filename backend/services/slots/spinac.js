...
async function processTransaction(
  ...
  if (method === "balance") {
    const balance = (await wiwiTokenToUsd(user.balance)) * exchangeRate;
  ...
  if (method === "debit") {
    ...
    let user_balance_cents = (await wiwiTokenToUsd(user.balance)) * 100;
    ...
    await updateAfterBet(
      user,
      await usdToWiwiToken(updatedUserBalanceCents / 100),
      txn_id,
      await usdToWiwiToken(amountCents / 100),
    ...
    if (!slotRes) {
      ...
      const betAmount = await usdToWiwiToken(amountCents / 100);
    ...
    await updateAfterFinish(
      user,
      await usdToWiwiToken(updatedUserBalance_cents / 100),
      txn_id,
      slotRes.amount,
      (await usdToWiwiToken(amountCents / 100)) / slotRes.amount,
      await usdToWiwiToken(amountCents / 100),
      await getSlotName(game_code),
      game_code,
      gameplay_final,
