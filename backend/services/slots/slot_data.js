...
async function initSlotData() {
  try {
    ...
    console.error(`Failed to load slot data from DB: ${e.message}`);
  }

  try {
    await loadRTP();
  } catch (e) {
    console.error(`Failed to load RTP: ${e.message}`);
  }

  try {
    await getSlots();
  } catch (e) {
    console.error(`Failed to get slots: ${e.message}`);
  }
...
async function loadRTP() {
  ...
  const fileName = process.env.SLOTEGATOR_RTP_FILE_NAME || "slotegatorRTP.xlsx";
  ...
  try {
    ...
    console.error(`Error loading RTP file (${fileName}): ${error.message}`);
    // If we failed with the env var, try the hardcoded one if different
    if (fileName !== "slotegatorRTP.xlsx") {
      console.log("Attempting fallback to slotegatorRTP.xlsx");
      try {
        const workbook = new ExcelJS.Workbook();
        await workbook.xlsx.readFile("slotegatorRTP.xlsx");
        ...
        logger.info("--- finished loading slots RTP (fallback) ---");
      } catch (err2) {
        console.error("Fallback RTP load failed:", err2.message);
      }
    }
  }
}
