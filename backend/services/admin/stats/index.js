...
const adminGetStats = async (io, socket, user, data, callback) => {
  try {
    ...
    const volumeMultiplier = settings?.general?.volumeMultiplier || 1;
    const fakeVolume = settings?.general?.fakeVolume || 0;
    ...
    const emptyGameStats = {
      crash: { wager: 0, won: 0 },
      coinflip: { wager: 0, won: 0 },
      mines: { wager: 0, won: 0 },
      plinko: { wager: 0, won: 0 },
      slide: { wager: 0, won: 0 },
      dice: { wager: 0, won: 0 },
      cases: { wager: 0, won: 0 },
      keno: { wager: 0, won: 0 },
      slots: { wager: 0, won: 0 },
      reme: { wager: 0, won: 0 },
      towers: { wager: 0, won: 0 },
    };

    const defaultPeriod = (start) => ({
      wager: 0,
      won: 0,
      deposit: 0,
      withdraw: 0,
      start,
      games: { ...emptyGameStats },
    });

    let statsResult = stats[0] || null;

    const multiplyStats = (obj) => {
      if (!obj) return obj;
      const multiplied = { ...obj };
      if (typeof multiplied.wager === "number")
        multiplied.wager *= volumeMultiplier;
      if (typeof multiplied.won === "number")
        multiplied.won *= volumeMultiplier;
      if (multiplied.games) {
        multiplied.games = Object.keys(multiplied.games).reduce((acc, game) => {
          acc[game] = {
            wager: multiplied.games[game].wager * volumeMultiplier,
            won: multiplied.games[game].won * volumeMultiplier,
          };
          return acc;
        }, {});
      }
    ...
    const finalStats = statsResult
      ? {
          total: multiplyStats({
            ...statsResult.total,
            wager: (statsResult.total.wager || 0) + fakeVolume,
            games: statsResult.total?.games || emptyGameStats,
          }),

          daily: multiplyStats({
            ...statsResult.daily,
            start: dailyStart,
            games: statsResult.daily?.games || emptyGameStats,
          }),
          weekly: multiplyStats({
            ...statsResult.weekly,
            start: weeklyStart,
            games: statsResult.weekly?.games || emptyGameStats,
          }),
          monthly: multiplyStats({
            ...statsResult.monthly,
            start: monthlyStart,
            games: statsResult.monthly?.games || emptyGameStats,
          }),
        }
      : {
          total: multiplyStats(defaultPeriod(null)),
          daily: multiplyStats(defaultPeriod(dailyStart)),
          weekly: multiplyStats(defaultPeriod(weeklyStart)),
          monthly: multiplyStats(defaultPeriod(monthlyStart)),
        };

    const count = await generalGetChatOnlineCount(io);

    callback({ success: true, stats: finalStats, onlineCount: count });
  } catch (err) {
    callback({
      success: false,
