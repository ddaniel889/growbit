...
const updateVolumeMultiplier = async () => {
  try {
    const Setting = require("../../../database/models/Setting");
    const settings = await Setting.findOne({}).lean();
    volumeMultiplier = settings?.general?.volumeMultiplier || 1;
  } catch (err) {
    logger.error("Failed to update volume multiplier:", err);
  }
};

setInterval(updateVolumeMultiplier, 60000);
setTimeout(updateVolumeMultiplier, 5000);

const generalAddBetsList = (io, bet, user, toUserOnly = false) => {
  try {
    // Apply volume multiplier to live feed bets
    const displayedBet = { ...bet };
    displayedBet.amount *= volumeMultiplier;
    if (displayedBet.payout) displayedBet.payout *= volumeMultiplier;

    displayedBet.user = generalUserGetFormated(user);

    if (toUserOnly) {
      io.of("/general").to(user._id.toString()).emit("bet", {
        bet: displayedBet,
      });
      return;
    }
    ...
    generalBets.all.unshift(displayedBet);
    ...
    if (displayedBet.amount >= process.env.WHALE_REQUIREMENT) {
      generalBets.whale.unshift(displayedBet);
    ...
    if (displayedBet.multiplier >= process.env.LUCKY_REQUIREMENT) {
      generalBets.lucky.unshift(displayedBet);
...
const generalBetsInit = async () => {
  try {
    if (mongoose.connection.readyState !== 1) {
      console.warn(
        "MongoDB not connected. Skipping General Bets initialization.",
      );
      return;
    }
    ...
    await updateVolumeMultiplier();

    const multiplyBet = (bet) => {
      const b = { ...bet };
      b.amount *= volumeMultiplier;
      if (b.payout) b.payout *= volumeMultiplier;
      return b;
    };

    // IMPORTANT : sanitize seeds and multiply
    generalBets.all = generalSanitizeBets(all.slice(0, 15).map(multiplyBet));
    generalBets.whale = generalSanitizeBets(
      whale.slice(0, 15).map(multiplyBet),
    );
    generalBets.lucky = generalSanitizeBets(
      lucky.slice(0, 15).map(multiplyBet),
    );
  } catch (err) {
    logger.error(err);
  }
