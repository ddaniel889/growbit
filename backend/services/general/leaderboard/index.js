    ...
    if (createdRace) {
      ...
      if (ioRef) {
        generalChatAddMessage(ioRef, {
          room: "en",
          message: `Weekly race has been finished, fastest player has won ${endingRace.winners[0].prize?.toFixed(
            0
          )} DLS`,
          type: "race",
        });
...
const raceOnStartup = async (io) => {
  ...
  try {
    if (mongoose.connection.readyState !== 1) {
      console.warn(
        "MongoDB not connected. Skipping Race on Startup initialization."
      );
      return;
    }
