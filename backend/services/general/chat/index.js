  ...
  try {
    fs.readFile("./chat.json", "utf8", (err, data) => {
      if (err) {
        ...
        try {
          if (!generalChatMessages?.en?.length && data && data.trim()) {
            try {
              ...
              logger.error("Error parsing chat data JSON:", innerParseErr);
              generalChatMessages = { en: [] };
            }
          }
        } catch (parseErr) {
    ...
    if (mongoose.connection.readyState === 1) {
      ...
      console.warn("MongoDB not connected. Skipping Chat Filter load.");
      generalChatFilter = [];
    }
