...
const getCryptoAddressesAndPrices = async (
  ...
  try {
    ...
    const supported = new Set(
      supportedCurrencies.map((cp) => cp.name + "-" + cp.network),
    );
    ...
    const NOT_CREATED = supportedCurrencies.filter(
    ...
    if (NOT_CREATED.length > 0) {
      ...
      if (createdAddresses.length !== NOT_CREATED.length) {
        return callback({
          success: false,
          error: {
            type: "error",
            message:
              "Failed to connect to the payment provider. Please check your server's network connection.",
          },
        });
      }
...
function getFee(cryptoPrice) {
  if (!cryptoPrice || !cryptoPrice.info || !cryptoPrice.info.networkList)
    return 0;
...
const createCryptoWithdrawRequest = async (
  ...
  try {
    ...
    if (!cryptoPrice || !cryptoPrice.price) {
      throw new Error(
        "Could not fetch crypto price. Please try again in a few minutes.",
      );
    }
...
const cashierCryptoCheckPricesByOxapay = async () => {
  try {
    ...
    const NOT_CREATED = Array.from(
      new Set(
        supportedCurrencies.map((c) => c.name).filter((c) => !inDb.has(c)),
      ),
    ).map((name) => ({ name: name }));
