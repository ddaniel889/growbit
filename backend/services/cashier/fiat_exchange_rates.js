...
const { wiwiTokenToUsd } = require("./growtopia/currency_service");
...
async function getFiatRatesDlsBase() {
  const dlsRate = await wiwiTokenToUsd(1);
  ...
  if (ratesToDollar) {
    for (let fiat of SUPPORTED_FIAT) {
      if (fiat !== "USD" && ratesToDollar[fiat]) {
...
async function updateRates() {
  try {
    ...
    try {
      ...
      console.warn(
        "Could not save exchange rates (likely collection limit reached):",
        e.message
      );
    }
    ...
    logger.error(`Update Rates Error: ${e.message}`);
  }
}
