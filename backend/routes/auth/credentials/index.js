  ...
  router.post("/", rateLimiterStrictMiddleware, async (req, res) => {
    if (mongoose.connection.readyState !== 1) {
      return res.status(503).json({
        ...
        error: {
          type: "error",
          message: "Login unavailable: Services are currently offline.",
        },
      });
    }
    try {
      ...
      userDatabase = await User.findByIdAndUpdate(userDatabase._id, {
        ...
        .select(
          "local.email local.emailVerified  discord.id username avatar rank balance wallet xp vault stats rakeback fair anonymous mute ban verifiedAt updatedAt createdAt"
        )
        .lean();
  ...
  router.post("/register", rateLimiterStrictMiddleware, async (req, res) => {
    if (mongoose.connection.readyState !== 1) {
      return res.status(503).json({
        ...
        error: {
          type: "error",
          message: "Registration unavailable: Services are currently offline.",
        },
      });
    }
    let session = null;
