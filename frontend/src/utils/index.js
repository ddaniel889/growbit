...
export function getOriginalsConfigAndStoreThem(games) {
  ...
  let maxWin = (originals[0] && originals[0].maxWin) || 10000;
  ...
  let minesMaxBet = (mines && +mines.maxBet) || 500;
  let crashMaxBet = (crash && +crash.maxBet) || 500;
  let slideMaxBet = (slide && +slide.maxBet) || 500;
  let kenoMaxBet = (keno && +keno.maxBet) || 500;
  let towersMaxBet = (towers && +towers.maxBet) || 500;
  let diceEdge = (dice && +dice.houseEdge) || 4;
  let plinkoEdge = (plinko && +plinko.houseEdge) || 4;
  let minesEdge = (mines && +mines.houseEdge) || 4;
  let kenoEdge = (keno && +keno.houseEdge) || 4;
  let towersEdge = (towers && +towers.houseEdge) || 4;
...
export function canClaimKey(user, level) {
  ...
  let timesClaimed =
    (user.claimedKeys &&
      user.claimedKeys.filter((num) => num === level).length) ||
    0;
...
function getKeysEarned(user, level) {
  if (!user || !user.stats || !user.stats.bet) {
    return 0;
  }
