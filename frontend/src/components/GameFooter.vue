        ...
        <div v-if="maxBet || game.maxBet">
          ...
          <span>
            {{
              (
                (maxBet || game.maxBet) *
                (fiatRates.data && fiatRates.data[selectedCurrency]
                  ? fiatRates.data[selectedCurrency]
                  : 1)
...
export default {
  ...
  computed: {
    ...
    game() {
      let g = this.gameList
        ? this.gameList.filter((g) => g.id === this.name)[0]
        : null;
    ...
    isOriginal() {
      return this.game && this.game.type === "Originals";
    },
  },
  created() {},
  methods: {
