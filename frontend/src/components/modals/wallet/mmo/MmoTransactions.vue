...
export default {
    ...
    if (!this.activeTicket) {
      ...
      axios
        ...
        .then(({ data }) => {
          const rate =
            (data.exchangeRates &&
              data.exchangeRates.filter(
                (e) => e.currency === this.currency.id
              )) ||
            [];
          const selectedRate = rate.length > 0 ? rate[0] : null;
          if (selectedRate) {
            this.loadingExchangeRates = false;
            this.dollarToCurrency = selectedRate.dollarToCurrency;
            this.currencyToDollar = selectedRate.currencyToDollar;
          } else {
            this.loadingExchangeRates = true;
          }
        })
  ...
  computed: {
    ...
    activeTicket() {
      const tickets = (this.mmoCashier && this.mmoCashier.tickets) || [];
