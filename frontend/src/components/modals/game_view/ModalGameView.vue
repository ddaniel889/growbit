      ...
      <div class="items-slot">
        ...
        <div class="item">
          ...
          <span class="outpot">
            {{
              (modalsData.game &&
                modalsData.game.user &&
                modalsData.game.user.username) ||
              "Hidden"
            }}
          </span>
        </div>
      ...
      <div class="result" v-if="isOriginal">
        ...
        <div class="keno-nums" v-else-if="modalsData.game.method === 'keno'">
          <div
            ...
            :class="{
              hit:
                modalsData.game.data.picks &&
                modalsData.game.data.picks.includes(num),
            }"
          >
            {{ num }}
          </div>
      ...
      if (this.isOriginal) {
        if (this.seedServer) {
          this.modalsSetData({
            ...
            data: this.modalsData.game && this.modalsData.game.data,
            seedServer: this.seedServer,
            seedPublic: this.seedPublic || this.seedClient,
            seedClient: this.seedClient || this.seedPublic,
            nonce: this.nonce,
            mines:
              this.modalsData.game.deck &&
              this.modalsData.game.deck.reduce(
                (total, x) => (x === "mine" ? total + 1 : total),
                0
              ),
            minesGrid: this.modalsData.game.gridSize || 25,
            plinkoRows:
              this.modalsData.game.data && this.modalsData.game.data.rows,
            plinkoRisk:
              this.modalsData.game.data && this.modalsData.game.data.risk,
  ...
  computed: {
    ...
    slotInfo() {
      ...
      return this.gameList
        ? this.gameList.filter((g) => g.id_hash === gameCode)[0]
        : null;
    },
    nonce() {
      return (
        (this.modalsData.game &&
          this.modalsData.game.fair &&
          this.modalsData.game.fair.nonce) ||
        (this.modalsData.game &&
          this.modalsData.game.fair &&
          this.modalsData.game.fair.seed &&
    ...
    seedPublic() {
      ...
      return (
        (this.modalsData.game &&
          this.modalsData.game.game &&
          this.modalsData.game.game.fair &&
    ...
    seedClient() {
      return (
        ...
        (this.modalsData.game &&
          this.modalsData.game.game &&
          this.modalsData.game.game.fair &&
          this.modalsData.game.game.fair.seed &&
          this.modalsData.game.game.fair.seed.seedClient)
      );
    },
    outcome() {
      if (this.modalsData.game && this.modalsData.game.side) {
      ...
      return this.modalsData.game && this.modalsData.game.outcome;
    },
    hashedSeedServer() {
      return (
        (this.modalsData.game &&
    ...
    blockNum() {
      ...
      return (
        this.modalsData.game &&
        this.modalsData.game.fair &&
        this.modalsData.game.fair.blockNum
      );
    },
  },
};
