  ...
  <div class="admin-user-transactions-element">
    ...
    <div class="element-section section-amount">
      ...
      <div class="section-content">
        ...
        <div
          class="content-value"
          v-bind:class="{ 'value-positive': adminGetAmount > 0 }"
        >
          <span>{{ adminFormatValue(adminGetAmount) }}</span>
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  computed: {
    ...
    adminGetMethod() {
      let method = this.transaction.method;

      if (this.transaction.method === "tip") {
        method =
          this.modalsData.user._id === this.transaction.sender.user._id
            ? "Tip Sent"
            : "Tip Received";
    ...
    adminGetAmount() {
      ...
      if (
        ...
        this.modalsData.user._id === this.transaction.sender.user
      ) {
        amount = -amount;
