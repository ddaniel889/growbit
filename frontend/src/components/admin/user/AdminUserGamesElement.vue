  ...
  <div class="admin-user-games-element">
    ...
    <div class="element-section section-amount">
      ...
      <div class="section-content">
        ...
        >
          <span>{{ adminFormatValue(game.payout - adminGetAmount) }}</span>
        </div>
      </div>
    </div>
  </div>
  ...
  computed: {
    adminGetAmount() {
      ...
      if (this.game.method === "blackjack") {
        amount = Math.floor(
          this.game.amount.main +
            this.game.amount.sideLeft +
            this.game.amount.sideRight
        );
        if (this.game.actions.includes("split") === true) {
          amount = Math.floor(amount + this.game.amount.main);
        }
      }
