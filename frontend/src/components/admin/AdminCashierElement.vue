<template>
  <div class="admin-cashier-element">
    <div class="element-section section-user">
      ...
      <div class="section-content">
        <div class="content-avatar">
          <AvatarImage :avatarNumber="transaction.user.avatar" />
        </div>
        <div v-html="transaction.user.username" class="content-username"></div>
        <div
          class="content-rank"
          v-bind:class="['rank-' + transaction.user.rank]"
        >
          {{ transaction.user.rank.toUpperCase() }}
        </div>
      </div>
    </div>
    ...
    <div
      class="element-section section-type"
      v-bind:class="['type-' + transaction.type]"
    >
    ...
    <div class="element-section section-amount">
      ...
      <div class="section-content">
        ...
        <div class="content-value">
          <span>{{ adminFormatValue(transaction.amount) }}</span>
        </div>
      </div>
    </div>
    <div class="element-section section-option">
      ...
      <div class="section-content">
        <button
          v-if="transaction.method === 'crypto'"
          v-on:click="adminApproveButton()"
        >
          Approve or Deny
        </button>
      </div>
...
.admin-cashier-element
  .element-section.section-type.type-deposit
  .section-content {
  color: var(--purple);
}

.admin-cashier-element
  .element-section.section-type.type-withdraw
  .section-content {
  color: var(--red);
}
