...
const mutations = {
  ...
  general_set_banners_loading(state, status) {
    state.banners.loading = status;
  },
...
const actions = {
  ...
  async generalFetchBanners({ commit, dispatch }) {
    commit("general_set_banners_loading", true);
    try {
      const response = await axios.get("/banners");
      if (response.data.success) {
        commit("general_set_banners", response.data.banners);
      }
    } catch (error) {
      console.error("Failed to fetch banners:", error);
    } finally {
      commit("general_set_banners_loading", false);
    }
  },
